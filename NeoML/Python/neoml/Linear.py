...
class LinearClassifier(PythonWrapper.Linear) :
	...
	def train(self, X, Y, weight=None):
		...
		return LinearClassificationModel(super()._train_classifier(x.indices, x.data, x.indptr, int(x.shape[1]), y, weight))
...
class LinearRegressor(PythonWrapper.Linear) :
	...
	def train(self, X, Y, weight=None):
		...
		return LinearRegressionModel(super()._train_regressor(x.indices, x.data, x.indptr, int(x.shape[1]), y, weight))
