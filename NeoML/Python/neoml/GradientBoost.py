...
class GradientBoostClassifier(PythonWrapper.GradientBoost):
	...
	def train(self, X, Y, weight=None):
		...
		if all(value >= 0 for value in y) != True:
			raise ValueError('All `Y` elements must be >= 0.')

		if all(value >= 0 for value in weight) != True:
			raise ValueError('All `weight` elements must be >= 0.')

		return GradientBoostClassificationModel(super()._train_classifier(x.indices, x.data, x.indptr, int(x.shape[1]), y, weight)) 
...
class GradientBoostRegressor(PythonWrapper.GradientBoost):
	...
	def train(self, X, Y, weight=None):
		...
		return GradientBoostRegressionModel(super()._train_regressor(x.indices, x.data, x.indptr, int(x.shape[1]), y, weight)) 
